...
/***************************************************************
//////////////////////////////////////////////////////////////////////////////////
local predictor config  /////Update...
****************************************************************////////////////////////////////////////
static int local_nelt = 4;
static int local_config[4] =
  { /* l1size */32, /* l2size */1024, /* hist */2, /* merge */0};


/***************************************************************
//////////////////////////////////////////////////////////////////////////////////
3-bit predictor config  /////Update...
****************************************************************////////////////////////////////////////
static int trimod_nelt = 1;
static int trimod_config[1] =
  { /* trimod tbl size */2048 };


///n-bit predictor config  /////Update...
///****************************************************************////////////////////////////////////////
static int nmod_nelt = 2;
static int nmod_config[2] =
  { /* trimod tbl size */2048,/*no of bits*/4};
...
static int neural_nelt = 3;
static int neural_config[3] = 
  {/* nr of perceptrons */1024, /* nr of weight bits */6, /* history length */16  };//////////////////////////////////////////////////////
...
****************************************************************/ /////////////////////////////////////////////////////////////////
  ...
  opt_reg_int_list(odb, "-bpred:trimod",
		   "trimodal predictor config (<table size>)",
		   trimod_config, trimod_nelt, &trimod_nelt,
		   /* default */trimod_config,
		   /* print */TRUE, /* format */NULL, /* !accrue */FALSE);

	opt_reg_int_list(odb, "-bpred:nmod",
		   "n-modal predictor config (<table size> <number_bits>)",
		   nmod_config, nmod_nelt, &nmod_nelt,
		   /* default */nmod_config,
		   /* print */TRUE, /* format */NULL, /* !accrue */FALSE);


   opt_reg_int_list(odb, "-bpred:neural",
		   "neural predictor config " 
	           "(<nr_of_perceptrons> <nr_of_weight_bits> <history_length>)",
		   neural_config, neural_nelt, &neural_nelt,
		   /* default */neural_config,	
                   /* print */TRUE, /* format */NULL, /* !accure */FALSE);

	  opt_reg_int_list(odb, "-bpred:local",
                   "local predictor config "
		   "(<l1size> <l2size> <l1_width> <merge>)",
...
/***************************************************************
Trimod parameter passing and function call
****************************************************************//////////////////////////////////////////////////////////////////
else if (!mystricmp(pred_type, "trimod"))
    {
      /* trimodal predictor, bpred_create() checks BTB_SIZE */
      if (trimod_nelt != 1)
	fatal("bad trimod predictor config (<table_size>)");
      if (btb_nelt != 2)
	fatal("bad btb config (<num_sets> <associativity>)");

      /* trimodal predictor, bpred_create() checks BTB_SIZE */
      pred = bpred_create(BPred3bit,
			  /* trimod table size */trimod_config[0],
			  /* 2lev l1 size */0,
			  /* 2lev l2 size */0,
			  /* meta table size */0,
			  /* history reg size */0,
...
/***************************************************************
Perceptron parameter passing and function call
****************************************************************//////////////////////////////////////////////////////////////////////////
   else if (!mystricmp(pred_type, "neural"))
    {
      /* neural predictor, bpred_create() checks BTB_SIZE */
      if (neural_nelt != 3)
	fatal("bad bimod predictor config (<nr_of_perceptrons> <nr_of_weight_bits> <history_length>)");
      if (btb_nelt != 2)
	fatal("bad btb config (<num_sets> <associativity>)");
	  pred = bpred_create(BPredPerceptron,
			  /* nr of perceptrons */ neural_config[0],
			  /* nr of weight bits */ neural_config[1],0,0,	
			  /* history length */    neural_config[2],0,
			  /* btb sets */btb_config[0],
			  /* btb assoc */btb_config[1],
			  /* ret-addr stack size */ras_size);
    }
	...
	else if (!mystricmp(pred_type, "nmod"))
    {
      /* neural predictor, bpred_create() checks BTB_SIZE */
      if (nmod_nelt != 2)
	fatal("bad nmod predictor config (<nr_of_perceptrons> <nr_of_weight_bits> <history_length>)");
      if (btb_nelt != 2)
	fatal("bad btb config (<num_sets> <associativity>)");
	  pred = bpred_create(BPrednbit,
			  /* nmod table size */nmod_config[0],
			  /* 2lev l1 size */nmod_config[1],
			  /* 2lev l2 size */0,
			  /* meta table size */0,
			  /* history reg size */0,
			  /* history xor address */0,
...
/***************************************************************
Local predictor parameter passing and function call
****************************************************************//////////////////////////////////////////////////////////////////////////
 

	else if (!mystricmp(pred_type, "local"))
    {
      /* local adaptive predictor, bpred_create() checks args */
      if (local_nelt != 4)
	fatal("bad local pred config (<l1size> <l2size> <l1_width> <merge>)");
      if (btb_nelt != 2)
	fatal("bad btb config (<num_sets> <associativity>)");

      pred = bpred_create(BPred2Level,
			  /* bimod table size */0,
			  /* local l1 size */local_config[0],
			  /* local l2 size */local_config[1],
			  /* meta table size */0,
			  /* l1_width*/local_config[2],
			  /* merge address */local_config[3],
			  /* btb sets */btb_config[0],
			  /* btb assoc */btb_config[1],
			  /* ret-addr stack size */ras_size);
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    fatal("cannot parse hello bro predictor type `%s'", pred_type);
